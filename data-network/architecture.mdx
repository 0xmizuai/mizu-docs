...
The MIZU Data Network is the very first decentralized data generation network, designed to enable developers to deploy and run data pipelines in a transparent, verifiable, and secure manner. The network is composed of three main components: the Core Network, the Verifiable Inference Network, and the AVS (Attestation, Verification, and Slashing) Layers.
...
The core of the MIZU Data Network is a DePIN network where all nodes run a DataVM (Data Virtual Machine). The DataVM is similar to the Ethereum Virtual Machine (EVM) but is designed specifically for deploying and running data pipelines instead of smart contracts.
